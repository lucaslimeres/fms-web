...
const MainLayout: React.FC = () => {
  ...
  const accessibleNavItems = navItems.filter(item => !item.adminOnly || user?.role === 'admin');
    ...
    <div className="flex h-screen bg-gray-100">
          ...
          <ul>
            {accessibleNavItems.map((item) => (
              <li key={item.path} className="mb-2">
                <NavLink to={item.path} end onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex items-center p-2 rounded-lg text-base font-normal transition-colors ${ isActive ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700' }`}>
                  <item.icon size={20} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between p-4 bg-white border-b md:justify-end">
            ...
            {/* Espaço reservado no cabeçalho para outros itens, se necessário */}
        </header>
