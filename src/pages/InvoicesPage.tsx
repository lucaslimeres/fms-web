...
const InvoicesPage: React.FC = () => {
    ...
    const totalAmount = useMemo(() => {
        return invoices.map(invoice => {
            ...
            const pending = [...bills, ...groupExpenses].filter(e => e.status === 'pending').reduce((acc, exp) => acc + parseFloat(exp.amount), 0);
                                                            ...
                                                            <div className="px-4 pb-4">
                                                                <table className="w-full text-sm text-left text-gray-500">
                                                                    <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                                                                        <tr><th className="px-4 py-2">Descrição</th><th className="px-4 py-2">Categoria</th><th className="px-4 py-2 text-right">Valor</th><th className="px-4 py-2 text-center">Ações</th></tr>
                                                                    </thead>
                                                                    <tbody>
                                                                        {card.expenses.map((exp: any) => (
                                                                            <tr key={exp.expense_id} className="border-b"><td className="px-4 py-3 font-medium text-gray-800">{exp.description}</td><td className="px-4 py-3">{exp.category_name}</td><td className="px-4 py-3 text-right">R$ {parseFloat(exp.amount).toFixed(2)}</td><td className="px-4 py-3 text-center flex justify-center gap-4"><button onClick={() => handleOpenEditModal(exp)} className="text-blue-600 hover:text-blue-800" title="Editar despesa"><Pencil size={18} /></button><button onClick={() => handleDeleteExpense(exp.expense_id)} className="text-red-600 hover:text-red-800" title="Excluir despesa"><Trash2 size={18} /></button></td></tr>
                                                                        ))}
                                                                    </tbody>
                                                                </table>
                                                            </div>
